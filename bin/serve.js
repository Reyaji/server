...
const updateCheck = async (isDebugging) => {
	...
	console.log(`${chalk.bgRed('UPDATE AVAILABLE')} The latest version of \`rey\` is ${update.latest}`);
};

const getHelp = () => chalk`
  {bold.cyan rey} - Static file serving and directory listing

  {bold USAGE}

      {bold $} {cyan rey} --help
      {bold $} {cyan rey} --version
      {bold $} {cyan rey} folder_name
      {bold $} {cyan rey} [-l {underline listen_uri} [-l ...]] [{underline directory}]

      By default, {cyan rey} will listen on {bold 0.0.0.0:3000} and serve the
  ...
  {bold ENDPOINTS}
      ...
      For TCP ports on hostname "localhost":
          ...
          {bold $} {cyan rey} -l {underline 1234}

      For TCP (traditional host/port) endpoints:

          {bold $} {cyan rey} -l tcp://{underline hostname}:{underline 1234}

      For UNIX domain socket endpoints:

          {bold $} {cyan rey} -l unix:{underline /path/to/socket.sock}

      For Windows named pipe endpoints:

          {bold $} {cyan rey} -l pipe:\\\\.\\pipe\\{underline PipeName}
`;
...
const loadConfig = async (cwd, entry, args) => {
	...
	for (const file of files) {
		...
		if (file === 'rey.json' || file === 'package.json') {
			console.error(warning('The config files `rey.json` and `package.json` are deprecated. Please use `reya.json`.'));
		}
